...
def beamprop_CN(v_in, lam, dx, n, nd,  z_end, dz, output_step):
    ...
    L1=sps.diags([main_diag, off_diag, off_diag], [0, -1, 1], format='csr')
    L1=(1j/(2 * k_bar * dx**2))*L1

    L2=sps.diags(wx, 0, format='csr')
    L2=1j*L2

    L=L1+L2

    A=sps.eye(len(wx), format='csr') - (dz/2) * L
    B=sps.eye(len(wx), format='csr') + (dz/2) * L
