...
def fdtd_1d(eps_rel, dx, time_span, source_frequency, source_position,
    ...
    Ez_in=np.zeros((Nt+1, Nx), dtype=complex)
    Hy_in=np.zeros((Nt+1, Nx-1), dtype=complex)

    Ez=np.zeros((Nt+1, Nx), dtype=complex)
    Hy=np.zeros((Nt+1, Nx), dtype=complex)


    dt_E=dt/(eps_rel[1:-1]*eps0)
    dt_H=dt/mu0

    t=np.linspace(0, time_span, Nt)
    x=np.arange(Nx)*dx-(Nx-1)*dx/2.0

    for n in range (Nt):
        Ez_in[n+1, 1:-1]=Ez_in[n, 1:-1]+dt_E*(Hy_in[n, 1:]-Hy_in[n, :-1])/dx

        ts=dt*(n+0.5)-3*source_pulse_length
        js=np.exp(-(ts/source_pulse_length)**2) * np.exp(-2j*np.pi*source_frequency*dt*(n+0.5))

        if 0<source_position<Nx-1:
            Ez_in[n+1, source_position] -= dt_E * js

        Hy_in[n+1, :]=Hy_in[n, :]+(dt/mu0)*(Ez[n+1, 1:]-Ez_in[n+1, :-1])/dx

        Ez_in[n+1, 0]=0
        Ez_in[n+1, -1]=0
        Ez[n, :]=Ez_in[n, :]
        for i in range(1, Nx-1):
            Hy=0.25*(Hy_in[n, i-1]+Hy_in[n, i]+Hy_in[n-1, i-1]+Hy_in[n-1, i])
