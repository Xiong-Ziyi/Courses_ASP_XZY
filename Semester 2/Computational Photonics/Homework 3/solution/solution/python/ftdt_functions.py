"""Solution to the assignment of Computational Photonics seminar 6 - FDTD.
"""
...
###############################################################################
# Alternative implementations using explicit loops and just-in-time           #
# compilation with numba                                                      #
###############################################################################

def fdtd_1d_naive(eps_rel, dx, time_span, source_frequency, source_position,
                  source_pulse_length):
    """Computes the temporal evolution of a pulsed excitation using the
    1D FDTD method. The temporal center of the pulse is placed at a
    simulation time of 3*source_pulse_length. The origin x=0 is in the
    ...
    Naive implementation using explicit loops.
    ...
    for n in range(Niter):
        ...
        # step E
        for i in range(1, Nx):
            Ez[n + 1, i] = Ez[n, i] + e_factor / dx * (Hy[n, i] - Hy[n, i - 1]) / eps_rel[i]
        # apply source
        Ez[n + 1, source_ind] -= e_factor / eps_rel[source_ind] * j_source
        # step H
        for i in range(Nx):
            Hy[n + 1, i] = Hy[n, i] + h_factor / dx * (Ez[n + 1, i + 1] - Ez[n + 1, i])
...
def fdtd_1d_naive_accelerated(eps_rel, dx, time_span, source_frequency,
    ...
    Numba accelerated naive implementation.
    ...
    -------
        ...
        t  : 1d-array
            Time of the field output
    """
    # use just-in-time compilation to accelerate naive update loops
    ...
    Ez, Hy = fdtd_1D_loop(Delta_t, dx, e_factor, h_factor,
                          source_angular_frequency, source_pulse_length,
                          source_ind, t0, eps_rel, Ez, Hy)
...
# import just-in-time compilation decorator for the acceleration of the
# "naive" implementations that use explicit loops
try:
    from numba import njit
except:
    # if the import of numba fails we define a dummy decorator that raises
    # a runtime error when the decorated function is called
    import functools
    ...
    def njit(*args, **kwargs):
        def wrapper(fun):
            ...
            def wrapped(*args, **kwargs):
                raise RuntimeError('Numba is required for '
...
@njit()
def fdtd_1D_loop(Delta_t, dx, e_factor, h_factor,
                 source_angular_frequency, source_pulse_length, source_ind, t0,
                 eps_rel, Ez, Hy):
    Niter, Nx = Hy.shape
    Niter -= 1
    for n in range(Niter):
...
def fdtd_3d_naive_accelerated(eps_rel, dr, time_span, freq, tau, jx, jy, jz,
                              field_component, z_ind, output_step):
    """Computes the temporal evolution of a pulsed spatially extended current
    source using the 3D FDTD method. Returns z-slices of the selected
    field at the given z-position every output_step time steps. The pulse
    ...
    for n in range(Niter):
        ...
        # %% update Ex(t=(n+1)*dt) %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        U = step_Ex(Ex, Hy, Hz, jx, iepsx, source_factor, e_factor, dr)
        if ((n + 1) % output_step == 0) and (field_component == 'ex'):
            F[next_out, :, :] = fdtd_3d_interpolate_field(U[:, 1:-1, 1:-1],
                                                          z_ind,
                                                          field_component)
            next_out += 1
        Ex[:, :, :] = U
        ...
        # %% update Ey(t=(n+1)*dt) %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        U = step_Ey(Ey, Hx, Hz, jy, iepsy, source_factor, e_factor, dr)
        if ((n + 1) % output_step == 0) and (field_component == 'ey'):
            F[next_out, :, :] = fdtd_3d_interpolate_field(U[1:-1, :, 1:-1],
                                                          z_ind,
                                                          field_component)
        ...
        # %% update Ez(t=(n+1)*dt) %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        U = step_Ez(Ez, Hx, Hy, jz, iepsz, source_factor, e_factor, dr)
        if ((n + 1) % output_step == 0) and (field_component == 'ez'):
            F[next_out, :, :] = fdtd_3d_interpolate_field(U[1:-1, 1:-1, :],
                                                          z_ind,
                                                          field_component)
        ...
        # %% update Hx(t=(n+1.5)*dt) %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        U = step_Hx(Hx, Ey, Ez, h_factor, dr)
        if ((n + 1) % output_step == 0) and (field_component == 'hx'):
            F[next_out, :, :] = fdtd_3d_interpolate_field(
                f * (U + Hx)[1:-1, :, :], z_ind,
                field_component)
        ...
        # %% update Hy(t=(n+1.5)*dt) %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        U = step_Hy(Hy, Ex, Ez, h_factor, dr)
        if ((n + 1) % output_step == 0) and (field_component == 'hy'):
            F[next_out, :, :] = fdtd_3d_interpolate_field(
                f * (U + Hy)[:, 1:-1, :], z_ind,
                field_component)
        ...
        # %% update Hz(t=(n+1.5)*dt) %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        U = step_Hz(Hz, Ex, Ey, h_factor, dr)
        if ((n + 1) % output_step == 0) and (field_component == 'hz'):
            F[next_out, :, :] = fdtd_3d_interpolate_field(
                f * (U + Hz)[:, :, 1:-1], z_ind,
                field_component)
...
# explicit signature enables precompilation of the function
@njit(['c8[:,:,:](c8[:,:,:],c8[:,:,:],c8[:,:,:],c8[:,:,:],f4[:,:,:],c8,f4,f4)'])
def step_Ex(Ex, Hy, Hz, jx, iepsx, source_factor, e_factor, dr):
    Nx, Ny, Nz = Ex.shape
    U = np.zeros(Ex.shape, dtype=Ex.dtype)
    f = e_factor / dr
    for i in range(Nx):
        for j in range(1, Ny - 1):
            for k in range(1, Nz - 1):
                U[i, j, k] = f * (Hz[i, j, k] - Hz[i, j - 1, k])
                U[i, j, k] -= f * (Hy[i, j, k] - Hy[i, j, k - 1])
                U[i, j, k] -= source_factor * jx[i, j, k]
                U[i, j, k] *= iepsx[i, j, k]
                U[i, j, k] += Ex[i, j, k]
    return U
...
# explicit signature enables precompilation of the function
@njit(['c8[:,:,:](c8[:,:,:],c8[:,:,:],c8[:,:,:],c8[:,:,:],f4[:,:,:],c8,f4,f4)'])
def step_Ey(Ey, Hx, Hz, jy, iepsy, source_factor, e_factor, dr):
    Nx, Ny, Nz = Ey.shape
    U = np.zeros(Ey.shape, dtype=Ey.dtype)
    f = e_factor / dr
    for i in range(1, Nx - 1):
        for j in range(Ny):
            for k in range(1, Nz - 1):
                U[i, j, k] = f * (Hx[i, j, k] - Hx[i, j, k - 1])
                U[i, j, k] -= f * (Hz[i, j, k] - Hz[i - 1, j, k])
                U[i, j, k] -= source_factor * jy[i, j, k]
                U[i, j, k] *= iepsy[i, j, k]
                U[i, j, k] += Ey[i, j, k]
    return U
...
# explicit signature enables precompilation of the function
@njit(['c8[:,:,:](c8[:,:,:],c8[:,:,:],c8[:,:,:],c8[:,:,:],f4[:,:,:],c8,f4,f4)'])
def step_Ez(Ez, Hx, Hy, jz, iepsz, source_factor, e_factor, dr):
    Nx, Ny, Nz = Ez.shape
    U = np.zeros(Ez.shape, dtype=Ez.dtype)
    f = e_factor / dr
    for i in range(1, Nx - 1):
        for j in range(1, Ny - 1):
            for k in range(Nz):
                U[i, j, k] = f * (Hy[i, j, k] - Hy[i - 1, j, k])
                U[i, j, k] -= f * (Hx[i, j, k] - Hx[i, j - 1, k])
                U[i, j, k] -= source_factor * jz[i, j, k]
                U[i, j, k] *= iepsz[i, j, k]
                U[i, j, k] += Ez[i, j, k]
    return U
...
# explicit signature enables precompilation of the function
@njit(['c8[:,:,:](c8[:,:,:],c8[:,:,:],c8[:,:,:],f4,f4)'])
def step_Hx(Hx, Ey, Ez, h_factor, dr):
    f = h_factor / dr
    Nx, Ny, Nz = Hx.shape
    U = np.zeros(Hx.shape, dtype=Hx.dtype)
    for i in range(1, Nx - 1):
        for j in range(Ny):
            for k in range(Nz):
                U[i, j, k] = Hx[i, j, k]
                U[i, j, k] -= f * (Ez[i, j + 1, k] - Ez[i, j, k])
                U[i, j, k] += f * (Ey[i, j, k + 1] - Ey[i, j, k])
    return U


# explicit signature enables precompilation of the function
@njit(['c8[:,:,:](c8[:,:,:],c8[:,:,:],c8[:,:,:],f4,f4)'])
def step_Hy(Hy, Ex, Ez, h_factor, dr):
    f = h_factor / dr
    Nx, Ny, Nz = Hy.shape
    U = np.zeros(Hy.shape, dtype=Hy.dtype)
    for i in range(Nx):
        for j in range(1, Ny - 1):
            for k in range(Nz):
                U[i, j, k] = Hy[i, j, k]
                U[i, j, k] -= f * (Ex[i, j, k + 1] - Ex[i, j, k])
                U[i, j, k] += f * (Ez[i + 1, j, k] - Ez[i, j, k])
    return U


# explicit signature enables precompilation of the function
@njit(['c8[:,:,:](c8[:,:,:],c8[:,:,:],c8[:,:,:],f4,f4)'])
def step_Hz(Hz, Ex, Ey, h_factor, dr):
    f = h_factor / dr
    Nx, Ny, Nz = Hz.shape
    U = np.zeros(Hz.shape, dtype=Hz.dtype)
    for i in range(Nx):
        for j in range(Ny):
            for k in range(1, Nz - 1):
                U[i, j, k] = Hz[i, j, k]
                U[i, j, k] -= f * (Ey[i + 1, j, k] - Ey[i, j, k])
                U[i, j, k] += f * (Ex[i, j + 1, k] - Ex[i, j, k])
    return U
